...
import com.buildman.buildman.adapter.Indent_Material_Adapter;
...
public class Indent_Material extends Activity {
	 private TextView mMatl_header_txv,mUnits_headertxv,mCur_Stk_header_txv,mReqd_Qty_header_txv;
...
//	 			Toast.makeText(getApplicationContext(), "go back", Toast.LENGTH_SHORT).show();
	 		 Intent i=new Intent(Indent_Material.this,Home.class);		
	 		startActivity(i);
	 		finish();
	 		return;
	 			}		
	...
	private void getList_fromDB() {
        ...
        LinearLayout relative = (LinearLayout) findViewById(R.id.listview_linearlayout_indent_matl);
        if(list.size()==0){
        ...
        for (Received_Material_Model cn : list) {
            ...
            String dc_invc = cn.getDC_Invoice_MST();
            String receivedStock = cn.getReceived_Stock_MST();
            String requiredStock = null;
            String rate = cn.getRate_MST();
            String display_flag=cn.getDisplay_Flag_MST();
            
            ArrayList<Received_Material_Model> db_list=db.find_SiteID_MaterialID_SynStatus_Material_Stk_TransList(selected_SiteID,material_id,"N");
				
				
				ArrayList<Received_Material_Model> indentQty_NotEqlZero_List=new ArrayList<Received_Material_Model>();
				for (Received_Material_Model pn : db_list) {
					 String indent_Qty=pn.getIndent_MST();					 
					  double  indent_Qty_double=Double.parseDouble(indent_Qty);                    
					  if(indent_Qty_double>0.00){                        	  						
						indentQty_NotEqlZero_List.add(pn);
					  }
				 }	
				if(indentQty_NotEqlZero_List.size()>0){
					for (int i=0;i<indentQty_NotEqlZero_List.size();i++) {
						Received_Material_Model recd_model=indentQty_NotEqlZero_List.get(i);  
						requiredStock=recd_model.getIndent_MST();
					}
            ...
            Received_Material_Model appadd = new Received_Material_Model(material,units,currentStock,dc_invc,receivedStock,requiredStock,rate);
            buildman_list.add(appadd);
            material_id_list.add(material_id);	 
            matl_name_list.add(material);
            currentStock_list.add(currentStock);
            units_list.add(units);
            reqd_Stock_list.add(requiredStock);
            rate_list.add(rate);
            display_flag_list.add(display_flag);
        }
		 Indent_Material_Adapter adapter = new
				 Indent_Material_Adapter(Indent_Material.this, R.layout.indent_material_row, buildman_list);
...
//             					"User-"+userType_inDB, Toast.LENGTH_SHORT).show();
	        	...
	        	Received_Material_Model received_model = (Received_Material_Model) parent.getItemAtPosition(position);
				            ...
				            Intent i=new Intent(Indent_Material.this,Indent_Material_Description.class);
				            ...
				            i.putStringArrayListExtra("reqdStock_list_send", reqd_Stock_list);
	 ...
	 private void getIds_FmLoginTable() {
		...
		//  get party_id and org_id fm Login table which always have only 1 row
	     db=new DatabaseHandler(Indent_Material.this);    
	...
	private void getIds() {
		...
		mListview=(ListView)findViewById(R.id.listview_indent_material);
